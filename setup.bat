...
:OPEN_WINDOWS
echo.
echo [5/6] main_control と enshutsu_overlay をボーダレスウィンドウで開きます...
echo         リールユニットはオーバーレイの液晶内に埋め込み済みです(単独ウィンドウは開きません)。
...
rem 筐体ビュー(reel.html)はオーバーレイが iframe で液晶内に読み込むので、単独ウィンドウでは開かない。
rem 単体で確認したいとき用に、見つかった場所のURLだけ最後に表示する。
rem (変数に & を入れると echo 時にコマンド区切りと解釈されるため、クエリは付けない)
set "REEL_URL="
...
echo.
echo  リールユニットはオーバーレイの液晶内に埋め込まれています。出し入れはオーバーレイの歯車 →
echo  「リール」タブ、または scripts\dev.cmd send reelIn / reelOut / reelToggle。表示状態は保存されます。
if defined CONTROL_URL echo  コンパネのURL: %BASEURL%/%CONTROL_URL%
if defined REEL_URL echo  筐体ビューを単体で開く場合: %BASEURL%/%REEL_URL%?mode=link^&hidebar=1
