...
:OPEN_WINDOWS
echo.
echo [5/6] main_control と enshutsu_overlay(と筐体ビュー reel)をボーダレスウィンドウで開きます...
...
rem 筐体ビュー(reel.html)の場所を探す。reel\ / enshutsu\ / ルート のどこに置いてもよい。
...
:CHECK_KYOTAI
rem 筐体ビュー(reel.html)。主制御と連動モード・操作パネル非表示で開く。任意ファイルなので無くても続行。
if defined KYOTAI_URL goto OPEN_KYOTAI
echo [情報] reel.html が見つからないため、筐体ビューはスキップします。
echo         reel\ / enshutsu\ / ルート のいずれかに置くと自動で開きます。
goto MAIN_BOARD
...
echo.
echo  筐体ビュー(reel.html)は主制御の1G結果を受けてリールが止まります。左上の歯車ボタンでローカル試打に切替可。
