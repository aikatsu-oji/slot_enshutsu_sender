...
│   │                            AT中は毎ゲーム上乗せ (AT_ADD_TABLE) とストック (AT_STOCK_TABLE) を抽選
│   │                            通常時はベル4連からAT抽選、5連以上はベルが揃うたびにストック
...
- **ベル連** (`BELL_CHAIN_AT` / `BELL_CHAIN_STOCK` / `BELL_CHAIN_AT_RATE`) …
  通常遊技でベルが揃い続けた回数を数え、**4連からAT抽選** (4連15% / 5連50% / 6連以上100%)、
  **5連以上はベルが揃うたびにストック+1** (抽選なしの確定)。当選したらほかと同じく前兆を挟む
  (契機名「ベル連」。自力で伸ばした実感があるぶん告知は短め)。
  ベル入賞は共通ベルと押順ベル正解のどちらでもよく (`BELL_PRIZES`)、揃わなかったゲームで0に戻る。
  **AT/GG中は数えないこと** — 押し順ナビで毎回揃うので連が無限に伸び、5連以上の確定ストックで
  出玉が発散する。カウントは `PLAY_NORMAL` (通常+前兆) のときだけで、前兆中はAT抽選をせず
  ストックだけ拾う (当選済みのため)。
  通常時のベル入賞率は約13%なので 4連が約1/4,000G・5連が約1/27,000G・6連以上はほぼ見ない。
  実測でベル連からのAT当選は全初当りの約2.4%、機械割を全設定一律で 0.6 ポイント押し上げる。
  副制御も `0x34` 全停止の表示役から同じ連数を数えていて (`SB_BELL_ZONE`)、3連以上で見立てを
  1段上げて「ベルが続いている」ことを予告の濃さで見せる (主制御の値は覗いていない。実測で写しは完全一致)。
- **AT中の上乗せとストック** (`AT_ADD_TABLE` / `AT_STOCK_TABLE` / `MainBoard.draw_table`) …
  AT中は毎ゲーム、成立役ごとの表から上乗せゲーム数とストック数を別々に引く
  (レア役では両方当たることもある)。AT初期ゲーム数は 30G で、上乗せで平均 53G まで伸びる。
  上乗せは AT 33G に1回・平均 +14G、ストックは AT 350G に1回くらい。
  **上乗せゲーム数は `0x53` のデータ1バイトに収めること** (最大 +200G。assert で担保)。
  この2つは出玉に直結するので、触ったら必ず `--ladder` を通す。設計上の落とし穴が2つある。
  - **共通ベルを上乗せ契機にしない**。共通ベルは AT 中の出玉の設定差そのもの
    (`LOTTERY_TABLE_AT` で設定1→6が約4倍) なので、上乗せを乗せると「払出が多い設定ほど
    AT も伸びる」と設定差が二重に効く。実測で設定1 94.9% / 設定6 122.8% まで開いた。
    上乗せの主契機は設定差の無い押順ベルとリプレイに置き、レア役で濃くする。
  - **ストックの主契機もレア役に置かない**。ストックは「1個増えると AT が1セット伸び、
    その間にまたストックを引ける」ループで、出玉は `1/(1 - 1セットのG数 × 1Gあたりの期待値)`
    で効く。分母が小さくなるほど跳ね上がるので、設定差のあるレア役 (設定1→6でスイカ+19%・
    チャンス目+28%) だけを契機にすると設定6のループだけが伸びる (連チャン 228G→271G、
    機械割 120%)。主契機は押順ベルに置く。ストックの頻度はこのループの安定性で頭打ちになり、
    上乗せのようには増やせない (上乗せ 1/515G→1/33G に対し、ストックは 1/494G→1/350G が上限)。
  ストックの供給源は GG中の赤7 (`GG_SEVEN_RATE`) と AT中抽選の2つで、合計が連チャン数を決める。
  AT中を効かせたぶん `GG_SEVEN_RATE` を 0.185→0.14 に下げ、連チャンの長さを据え置いてある。
  `LOTTERY_TABLE_AT` の共通ベルが機械割の水準を合わせる唯一のつまみ (+1000 で +1.1〜1.5 ポイント)。
- **前兆** (`ZENCHOU_GAMES` / `MainBoard._start_zenchou`) … レア役からのGG当選は即告知せず、
  ...
  前兆のぶん通常遊技が伸びるので**機械割が下がる**。前兆が無い場合に比べて設定1で 0.39・設定6で 0.67
...
- 演出の抽選は**副制御**にある (`SubBoard._draw_rank` / `_draw_pattern` / `_draw_freeze`)。主制御は演出に関与しない。
  ...
  **弱 (通常時の94%) の重みを緑以上へ置かないこと** — 母数が大きいので、置くと当選しないゲームの緑・赤が
  ...
  白 1/26 (1.3%) → 青 1/118 (5.3%) → 緑 1/380 (9.8%) → 赤 1/1483 (17.9%) → 金 1/3985 (93.4%) →
  ロック 1/7583 (100%) で、頻度・期待度とも単調。テーブルを触ったらこの単調性を測り直す。
