...
| Twitch アクション | 見せ方 | 投入枚数 | 通常時の回転数 | 実機ウェイトでの時間 |
| --- | --- | --- | --- | --- |
| 1 bit | - | 1 枚 | 約 0.85G | 3 秒 |
| 100 bits | 「85 回転」 | 100 枚 | 約 85G | 約 6 分 |
| 1,000 bits | 「850 回転」 | 1,000 枚 | 約 850G | 約 58 分 |
| サブスク tier1 | 「250 回転」 | 300 枚 | 約 256G | 約 17 分 |
| サブスク tier2 / tier3 | 「500 / 1000 回転」 | 600 / 1,200 枚 | 約 512G / 1,025G | 約 35 / 70 分 |
| サブギフト 1 個 | tier1 と同じ | 300 枚 | 約 256G | 約 17 分 |
| レイド 1 人 | 「17 回転」 | 20 枚 | 約 17G | 約 70 秒 |
| チャンネルポイント「100 回転」 | そのまま | 120 枚 | 約 100G | 約 7 分 |
| フォロー | 演出のみ | 0 枚 | - | - |
...
1. **換算レートの最終値** — 単位は 1 回転 = 3 枚 で確定。実効レート「1 回転 ≒ 1.2 枚」と
   4 章の換算表 (1 bit = 1 枚 / sub tier1 = 300 枚 / レイド 1 人 = 20 枚) でよいか。
   配信の長さが決まるいちばん重要な数値なので、初回は控えめ (視聴者の投入が要る側) から始めて調整する。
