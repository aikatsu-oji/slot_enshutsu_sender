...
| Twitch アクション | 見せ方 | 投入枚数 | 通常時の回転数 | 実機ウェイトでの時間 | 天井 (1200G) 比 |
| --- | --- | --- | --- | --- | --- |
| 1 bit | - | 1 枚 | 約 0.85G | 3 秒 | 0.1% |
| 100 bits | 「85 回転」 | 100 枚 | 約 85G | 約 6 分 | 7% |
| 1,000 bits | 「850 回転」 | 1,000 枚 | 約 850G | 約 58 分 | 71% |
| **サブスク tier1** | 「420 回転」 | **500 枚** | 約 427G | 約 29 分 | 36% |
| **サブスク tier2** | 「850 回転」 | **1,000 枚** | 約 855G | 約 58 分 | 71% |
| **サブスク tier3** | 「2100 回転」 | **2,500 枚** | 約 2,137G | 約 2.4 時間 | **178%** |
| サブギフト 1 個 | そのティアと同額 | 500 枚〜 | 約 427G〜 | 約 29 分〜 | 36%〜 |
| レイド 1 人 | 「17 回転」 | 20 枚 | 約 17G | 約 70 秒 | 1% |
| チャンネルポイント「100 回転」 | そのまま | 120 枚 | 約 100G | 約 7 分 | 8% |
| フォロー | 演出のみ | 0 枚 | - | - | - |

### ティア差は 1 : 2 : 5 で付ける

サブスクのティア比は **Twitch の実価格比 ($4.99 / $9.99 / $24.99) に合わせて 1 : 2 : 5**。
等倍 (1:2:3) や倍々 (1:2:4) にすると、上位ティアが価格に見合わない。

- **tier3 は 1 件で天井到達が確定する** (2,137G > 1,200G)。上位ティアに明確なご褒美を置ける。
- **tier1 でも天井の 1/3 強**。「あと 2 人で天井」が成立する刻みになる。
- ビッツと比べると、1 ドルあたり サブスクが約 1.4 倍お得。サブスクを優遇する形になっている
  (100 bits ≒ 1.4 ドルで 85G / tier1 ≒ 5 ドルで 427G)。

サブギフトは **ギフトされたティアと同額 × 個数**。ただし 1 イベント上限
(`maxMedalsPerEvent` = 5,000 枚) で頭を打たせる。10 連ギフトで 5,000 枚を超える分は `bank` へ。

**増額しても配信ループが死なない理由は下皿上限にある。** `reserveMax` を 3,000 枚
(≒ 2,560G ≒ 実機ウェイトで約 3 時間 = 1 配信ぶん) に絞り、超過分は `bank` に落として次回へ持ち越す。
これがないと tier3 が 1 件来た時点でその日の投入が全部無意味になる。
逆に言うと、**ティアの額を上げるときは必ず `reserveMax` とセットで考える**。
...
```json
{
  ...
  "rules": [
    ...
    { "name": "サブスク", "when": { "kind": "sub" },
      "medals": { "byTier": { "1": 500, "2": 1000, "3": 2500 } },
      "enshutsu": { "type": "banner", "rankByTier": { "1": "赤", "2": "赤", "3": "金" } } },

    { "name": "ギフト",   "when": { "kind": "subgift" },
      "medals": { "of": "amount", "unitByTier": { "1": 500, "2": 1000, "3": 2500 }, "max": 5000 } },
...
1. **換算レートの最終値** — 単位は 1 回転 = 3 枚 で確定。実効レート「1 回転 ≒ 1.2 枚」と
   4 章の換算表 (1 bit = 1 枚 / sub 500・1,000・2,500 枚 / レイド 1 人 = 20 枚) でよいか。
   tier3 が 1 件で天井到達 (178%) になる刻みなので、上位ティアの価値ははっきり出る。
   もっと重くするなら 1 : 2 : 5 の比を保ったまま全体を上げる (600 / 1,200 / 3,000 など)。
